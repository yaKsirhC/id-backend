...
async function loginController(req: Request, res: Response) {
    ...
    try {
        ...
        const user = await UserModel.findOne({ token }, { account: 0 }).lean().exec()
        ...
        return res.status(200).send({ user, token: tokenQ, refreshToken: refreshTokenQ })
...
async function refreshController(req: Request, res: Response) {
    try {
        ...
        if (!user) return res.status(500).send({ message: 'Token is not valid' })

        const token = generateToken(false, user._id.toString())
        const refreshTokenQ = generateToken(true, user._id.toString())
        return res.status(200).send({ token, refreshToken: refreshTokenQ })
