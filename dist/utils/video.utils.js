...
function createVideoProcess(outputUrl) {
    try {
        const outputStream = new PassThrough();
        const proc = ffmpeg();
        proc.input(outputStream).inputFormat('webm').inputOptions([
            '-re'
        ]).output(outputUrl).outputOptions([
            '-c:v libx264',
            '-preset veryfast',
            '-b:v 3000k',
            '-maxrate 50M',
...
function assignVideoProcessToUser(userId, streamKey) {
    ...
    if (!doc) {
        ...
        outputStreams.set(userId, {
            outputStream: outputStream,
            proc: proc
        });
    }
}
function streamVideo(video, userId) {
    console.log(video);
    const doc = outputStreams.get(userId);
    if (doc) {
...
function closeVideoProcess(userId) {
    const doc = outputStreams.get(userId);
    if (doc) {
...
export { createVideoProcess, assignVideoProcessToUser, streamVideo, closeVideoProcess };
