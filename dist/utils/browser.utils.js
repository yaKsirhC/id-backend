...
const launchBrowser = async (userId, username, password)=>{
    try {
        ...
        if (!exists) {
            ...
            const userInput = await page.$('#id_username');
            await userInput.type(username, {
                delay: 200
            });
            await new Promise((r)=>setTimeout(r, 1250));
            const passInput = await page.$('#id_password');
...
const validateUserID = async (user)=>{
    try {
        const handleJob = async (page, browser)=>{
        ...
        await new Promise((r)=>setTimeout(r, 2000));
        const userInput = await page.$('#id_username');
        await userInput.type(user.account_username, {
            delay: 200
        });
        await new Promise((r)=>setTimeout(r, 1250));
        const passInput = await page.$('#id_password');
        await passInput.type(user.account_password, {
            delay: 180
        });
        const btn = await page.$('input[type="submit"]');
        await new Promise((r)=>setTimeout(r, 1250));
