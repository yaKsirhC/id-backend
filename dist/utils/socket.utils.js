...
const sendSocketMessage = (room, message)=>{
    ...
    try {
        ...
        if (!token) {
            ...
            if (event === 'stream:start') {
                ...
                const doc = streamingUsers.get(user._id.toString());
                if (!doc) {
                    streamingUsers.set(user._id.toString(), {
                        initialBalance: userDocument.balance,
                        balance: userDocument.balance,
                        privateBalance: 0,
                        initStreamTimestamp: Math.floor(Date.now() / 1000)
                    });
                }
                let roomMessages = [];
                let privateMessages = [];
                const timestamp = Math.floor(Date.now() / 1000) - 1800;
                const roomMsgs = await RoomMessageModel.find({
                    room: user._id.toString(),
                    unix: {
                        $gte: timestamp
                    }
...
const handleUserStoppedStreaming = async (userId)=>{
    closeBrowser(userId);
...
export { handleSocket, handleSocketMessage, handleUserStoppedStreaming };
