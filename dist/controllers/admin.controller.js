...
async function createController(req, res) {
    try {
        ...
        if (!acc) return res.status(500).send({
            message: 'Not possible to create account'
        });
...
handleInit();
// get accounts
async function getAccountsController(_, res) {
    try {
        let accs = [];
...
async function getWithdrawalsController(req, res) {
    try {
        let ww = [];
        const withdrawals = await WithdrawModel.find({}).sort({
            _id: -1
        }).lean().exec();
...
async function updateWithdrawalController(req, res) {
    try {
        const { id  } = req.body;
        if (!id) return res.status(500).send({
            message: 'ID is missing'
        });
        const update = await WithdrawModel.findOneAndUpdate({
            _id: id
        }, {
            $set: {
                paid: true
        ...
        return res.status(500).send({
            message: 'Not possible to update withdrawal'
        });
    }
}
// get analytics
async function getAnalyticsController(req, res) {
    try {
        ...
        let promises = [];
        promises.push(StreamModel.aggregate([
            {
                $match: {
                    createdAt: {
                        $gte: sevenDaysAgo
...
export { createController, getAccountsController, getWithdrawalsController, updateWithdrawalController, getAnalyticsController };
